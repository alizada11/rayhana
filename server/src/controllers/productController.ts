...
export const createProduct = async (req: Request, res: Response) => {
  try {
    ...
    const normalizedAmazonCaUrl = normalizeProductUrl(amazonCaUrl);
    if (normalizedAmazonCaUrl === null) {
...
export const updateProduct = async (req: Request, res: Response) => {
  try {
    ...
    const normalizedAmazonCaUrl =
      amazonCaUrl !== undefined ? normalizeProductUrl(amazonCaUrl) : undefined;
    if (normalizedAmazonCaUrl === null) {
      return res.status(400).json({ error: "Invalid amazonCaUrl protocol" });
    }
