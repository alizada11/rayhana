...
import { ShoppingBag, Star, CheckCircle, Gift, ShieldCheck, Heart } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function Products() {
  ...
  const products = [
    {
      id: 1,
      name: t('products_page.pot_red'),
      category: 'cookware',
      image: '/images/home-hero-pot.jpg',
      price: t('products_page.price_7'), // Default price
      description: t('products_page.pot_desc'),
      rating: 5,
      sizes: [7, 9, 12],
      colors: ['#E63946'],
      prices: {
        7: t('products_page.price_7'),
        9: t('products_page.price_9'),
        12: t('products_page.price_12')
      }
    },
    {
      id: 5,
      name: t('products_page.pot_grey'),
      category: 'cookware',
      image: '/images/product-grey-original.jpg',
      price: t('products_page.price_7'), // Default price
      description: t('products_page.pot_desc'),
      rating: 5,
      sizes: [7, 9, 12],
      ...
      <div className="container">
        ...
        >
          {products.map((product) => (
            <ProductCard key={product.id} product={product} t={t} isRTL={isRTL} />
          ))}
        </motion.div>
        ...
        >
          ...
          <div className="space-y-6 text-lg leading-relaxed text-muted-foreground">
            <p>
              {t('products_page.brand_message_p1')}
            </p>
            ...
            <p>
              {t('products_page.brand_message_p2')}
            </p>
...
function ProductCard({ product, t, isRTL }: any) {
  ...
  return (
    <Dialog>
          ...
          <div className="p-6 space-y-4">
            <div>
              ...
              {product.sizes && (
                <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                  <div className="text-xs font-bold text-muted-foreground">{t('products_page.sizes')}:</div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border/50">
              <span className="text-xl font-bold text-primary">
                {currentPrice.includes('$') || currentPrice.includes('دلار') ? currentPrice : `${isRTL ? '' : '$'}${currentPrice}${isRTL ? ' تومان' : ''}`}
              </span>
              <Button 
                size="sm" 
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  const searchQuery = encodeURIComponent(`Rayhana Kitchen ${product.name} ${selectedSize ? `size ${selectedSize}` : ''}`);
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-serif text-2xl font-bold text-primary">{product.name}</DialogTitle>
          <DialogDescription>
            {product.description}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 p-4 bg-yellow-50/50 dark:bg-yellow-900/10 rounded-xl border border-yellow-100 dark:border-yellow-900/30">
              <div className="text-center">
                ...
                <div className="text-xs text-muted-foreground mt-1">{t('products_page.out_of_5')}</div>
              </div>
              <div className="h-12 w-px bg-border mx-2"></div>
              <div>
                <h4 className="font-bold text-lg">{t('products_page.amazon_reviews')}</h4>
                <p className="text-sm text-muted-foreground">Based on global ratings</p>
              </div>
            </div>
